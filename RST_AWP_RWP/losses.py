...
def trades_loss(model,
    ...
    if epoch >= awp_warmup:
        if robustWP:
            rwp = rwp_adversary.calc_awp(inputs_adv=x_adv, inputs_clean=x_natural, targets=y, beta=beta)
            rwp_adversary.perturb(rwp)
        else:
            awp = awp_adversary.calc_awp(inputs_adv=x_adv, inputs_clean=x_natural, targets=y, beta=beta)
    ...
    if robustWP:
        return loss, loss_natural, loss_robust, loss_entropy_unlabeled, rwp
    else:
        return loss, loss_natural, loss_robust, loss_entropy_unlabeled, awp
