...
class TradesRWP(object):
    ...
    def calc_awp(self, inputs_adv, inputs_clean, targets, beta):
        ...
        for ii in range(10):
            ...
            loss_robust = F.kl_div(F.log_softmax(self.proxy_2(inputs_adv), dim=1), F.softmax(self.proxy_2(inputs_clean), dim=1), reduction='none')
            loss = loss_natural + beta * loss_robust

            Indicator = (loss < 1.7).cuda().type(torch.cuda.FloatTensor)
